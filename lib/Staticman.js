...
const Staticman = function (parameters) {
  ...
  // Initialise GitHub or Gitlab API
  if (parameters.site === 'github') {
    this.gitApi = new GitHub({
      username: this.parameters.username,
      repository: this.parameters.repository,
      branch: this.parameters.branch
    })
  } else if (parameters.site === 'gitlab') {
    this.gitApi = new Gitlab({
      username: this.parameters.username,
      repository: this.parameters.repository,
      branch: this.parameters.branch
    })
...
Staticman.prototype._initialiseSubscriptions = function () {
  ...
  const subscriptions = new SubscriptionsManager(this.parameters, this.gitApi, mailgun)
...
Staticman.prototype.authenticate = function () {
  this.gitApi.authenticateWithToken(config.get('githubToken'))
}
...
Staticman.prototype.getSiteConfig = function (force) {
  ...
  return this.gitApi.readFile(this.configPath.file).then(data => {
...
Staticman.prototype.processEntry = function (fields, options) {
  ...
  return this.getSiteConfig().then(config => {
    ...
    if (this.siteConfig.get('moderation')) {
      ...
      return this.gitApi.writeFileAndSendPR(filePath, data, newBranch, commitMessage, this._generatePRBody(fields))
    } else if (subscriptions && options.parent) {
      subscriptions.send(options.parent, fields, options, this.siteConfig)
    }

    return this.gitApi.writeFile(filePath, data, this.parameters.branch, commitMessage)
  }).then(result => {
    return {
      fields: fields,
